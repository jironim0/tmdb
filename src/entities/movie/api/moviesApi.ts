...
import type { GenresResponse, MovieCreditsResponse, MovieDetail, TMDBResponse } from "./moviesApi.type";
...
export const movieApi = baseApi.injectEndpoints({
    endpoints: (build) => ({
        ...
        fetchMoviesByCategory: build.query<TMDBResponse, { category: string; page: number }>({
            query: ({ category, page }) => ({
                url: `movie/${category}`,
                params: { page }
            }),
        ...
        getMovies: build.query<TMDBResponse, { with_genres?: string, sort_by?: string, vote_average_gte?: number, vote_average_lte?: number, page: number }>({
            query: (params) => ({
                url: '/discover/movie',
                params: {
                ...params,
